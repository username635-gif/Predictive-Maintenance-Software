...
export const PIGComparisonModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const [pigRuns, setPigRuns] = useState<PIGRun[]>([]);
  ...
  useEffect(() => {
    ...
    api.getPigRuns()
      .then(res => {
        if (cancelled) return;
        const runs = res.pig_runs as PIGRun[];
        setPigRuns(runs);
        setRunAId(runs[1]?.id ?? runs[0]?.id ?? '');
        setRunBId(runs[0]?.id ?? '');
        setLoading(false);
      })
      .catch(err => {
        if (cancelled) return;
        setLoadError(err instanceof ApiError ? err.message : 'Failed to load PIG runs.');
        setLoading(false);
      });
    return () => { cancelled = true; };
  }, []);

  const runA = pigRuns.find((r) => r.id === runAId);
  const runB = pigRuns.find((r) => r.id === runBId);
    ...
    <div style={{
      ...
      <div className="animate-fade-in" style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px' }}>
          ...
          {loadError && (
            <div style={{ padding: '40px', textAlign: 'center', color: '#F06A50', fontSize: '13px' }}>
              {loadError}
            </div>
          )}

          {!loading && !loadError && pigRuns.length === 0 && (
            <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)', fontSize: '13px' }}>
              No PIG runs available yet.
            </div>
          )}
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
                      <thead>
                        <tr style={{ background: '#1A1C23' }}>
                          {['Mile', `${runA.date} (%WT)`, `${runB.date} (%WT)`, 'Growth (% yr-1)', '2yr Forecast (%)', 'Feature', 'Orientation'].map(h => (
